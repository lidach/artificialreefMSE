...
function(MSE_list)
{
  ...
  with(MSE_list, {
    ...
    # initial EM
    EM_init <- try(run_EM(sim_data = OM_init, 
                          lh_list = lh_list, 
                          fixed_list = EM_set$fixed_list, 
                          rand_list = "log_recruit_devs",
                          years_run = 1:OM_init$settings$nyears), silent = TRUE)
    ...
    for(em in 2:length(EM_run_years)){		
      OM_list <- run_future_OM(seed = seed_vec[em],
      ...
      EM_res <- try(run_EM(sim_data = OM_list, 
                          lh_list = lh_list, 
                          fixed_list = EM_set$fixed_list, 
                          rand_list = "log_recruit_devs",
                          years_run = 1:OM_list$settings$nyears), silent = TRUE)
      # get SSB trend
    ...
    sdReport <- summary(EM_res$sdReport)
    ...
    ## output list
    output <- NULL
      output$OM <- list(Depletion = OM_list$time_dyn$SSB.SSB0_save,
                        SSB = OM_list$time_dyn$SSB_save)
        output$OM$Data <- list(Catch = OM_list$EM_data$Catch_obs_save,
        ...
        output$OM$Spatial <- list(AR_sites = OM_list$spatial$AR_sites,
      ...
      output$EM <- list(parameters = EM_res$param_df,
